...
type ImageMessagePart = Extract<MessagePart, { type: "file" }> | any; // Allow both file and image parts
type FileMessagePart = Extract<MessagePart, { type: "file" }> | any; // Allow both file and image parts
...
export const UserImagePart = memo(function UserImagePart({
  part,
  isLast,
  message,
  setMessages,
  ...
  return (
    <div className="flex flex-col gap-2 items-end my-2">
      <div
        className={cn(
          "max-w-md overflow-hidden rounded-2xl",
          { "opacity-50": isError },
          isError && "border-destructive border",
        )}
      >
        ...
        <img
          src={
            (part as any).type === "image"
              ? (part as any).image
              : (part as any).url || (part as any).data
          }
          alt="Uploaded image"
...
export const UserFilePart = memo(function UserFilePart({
  part,
  isLast,
  message,
  setMessages,
  ...
  // Convert the file part to FileAttachment format for display
  const fileAttachment: FileAttachment = useMemo(() => {
    const filePart = part as any;
    const filename = filePart.name || filePart.url?.split("/").pop() || "file";

    // Handle different ways the file URL might be stored
    ...
    return {
      ...
      size: 0, // We don't have size info in the message part
      type: filePart.mediaType || "application/octet-stream",
      url: fileUrl || "#", // Fallback URL
  ...
  return (
    <div className="flex flex-col gap-2 items-end my-2">
      ...
      >
        <FileAttachmentDisplay file={fileAttachment} showPreview={true} />
      </div>
      {isLast && (
        <div className="flex w-full justify-end opacity-0 group-hover/message:opacity-100 transition-opacity duration-300">
          <Tooltip>
